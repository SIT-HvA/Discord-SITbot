...
- M11 `/events` krijgt een `view` keuze: `Compact` (default sinds T018) en `Full` (grote embeds met poster als afbeelding en alle velden). Compact is 1 kleine embed per event, gesorteerd op date: titel klikbaar naar `https://svsit.nl/events/<id>`, 1 regel description `<t:start:f>  locatie` met ` (ended)` als het event voorbij is, poster klein rechtsboven (thumbnail), rand in de categoriekleur. Geen omschrijving, geen velden, geen grote afbeelding, geen footer. Zelfde content-regel `Events this week: <ma> to <zo>` en dezelfde 10-embeds en 6000-tekens fit met noot als full. Compact kaarten zijn zoveel mogelijk even groot: gelijke breedte via een transparante spacer-afbeelding (attachment, 400x1) als `image` in elke kaart, gelijke hoogte via titel op 1 regel (afgekapt op 60) en description vast op 2 regels (tijd, dan locatie afgekapt op 60) binnen de 80 pixel van de thumbnail. Lege week: zelfde `No events this week.` Prefix: `%events [next] [full]`, woorden in elke volgorde (compact is default, `full` schakelt om).
...
- Compact als 1 lijst-embed of kleine embeds per event (2026-09-24)? Thijmen na de eerste versie: "plaatje klein rechts en nog een border om een lijst item heen". Dus kleine embeds per event met thumbnail; de lijst-embed (buildWeekListEmbed) is vervangen, niet als derde view gehouden.
- Gelijke grootte compact kaarten (2026-09-24)? Thijmen: "is het mogelijk om de lijst items dezelfde grote te geven", gekozen: spacer-image trick voor breedte plus vaste 2-regel layout voor hoogte. Bekend: geldt voor desktop, mobiel is altijd volle breedte; een event zonder poster heeft geen thumbnail en is iets lager, dat is geaccepteerd.
- Default view (2026-09-24, na de dev-klik)? Thijmen: "maak compact default". Compact is default op slash en prefix, `view:Full` of `%events full` geeft de grote kaarten. Eerdere keuze (full default) is hiermee vervangen.
