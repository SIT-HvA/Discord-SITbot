...
- M11 `/events` krijgt een `view` keuze: `Full` (default, huidig gedrag met grote embeds) en `Compact`. Compact is 1 kleine embed per event, gesorteerd op date: titel klikbaar naar `https://svsit.nl/events/<id>`, 1 regel description `<t:start:f>  locatie` met ` (ended)` als het event voorbij is, poster klein rechtsboven (thumbnail), rand in de categoriekleur. Geen omschrijving, geen velden, geen grote afbeelding, geen footer. Zelfde content-regel `Events this week: <ma> to <zo>` en dezelfde 10-embeds en 6000-tekens fit met noot als full. Compact kaarten zijn zoveel mogelijk even groot: gelijke breedte via een transparante spacer-afbeelding (attachment, 400x1) als `image` in elke kaart, gelijke hoogte via titel op 1 regel (afgekapt op 60) en description vast op 2 regels (tijd, dan locatie afgekapt op 60) binnen de 80 pixel van de thumbnail. Lege week: zelfde `No events this week.` Prefix: `%events [next] [compact]`, woorden in elke volgorde.
