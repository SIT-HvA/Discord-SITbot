...
| 2026-09-24 | `npm run deploy` na T018 (compact default, choices-volgorde) door Thijmen in de dev-guild | 5 commands. Thijmen: `/events` zonder optie werkt op dev. Productie volgt na merge van PR #3. |
| 2026-09-24 ~13:25 UTC | PR #3 gemerged (6f9eb41), pull plus deploy plus restart op sit-srv-01 door SIT (niet door Claude) | Thijmen: `/events` werkt in de SIT-server. Productie draait increment 5 (compact default, full als optie). |

## Rollback
Vorige werkende commit: 069dad6 (increment 4, /events alleen full), daarvoor d0b6110 (increment 3, zonder /events; /events weghalen: `git checkout d0b6110 -- scripts lib test` en `npm run deploy`), daarvoor ca2c2bf (increment 2 zonder announce), daarvoor 6f0b645 (increment 1 zonder vertaling), daarvoor b8964a2 (zonder /event). Terug: `git checkout b8964a2 -- scripts lib` en opnieuw `npm run deploy` (registreert dan 3 commands, /event verdwijnt uit de guild).
