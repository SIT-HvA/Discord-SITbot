...
//
//   join            -> ping in #introductions, rules delivered ephemerally
//   first message   -> lid role granted, congratulations ephemerally in #general
//
// Both replies are ephemeral ("Only you can see this"), so neither clutters the
// channel. That flag needs an interaction token, and neither a join nor a plain
// message has one — so each step posts a public prompt that pings the member,
// and the *button click* (which is an interaction) carries the ephemeral. Same
// constraint, same workaround as `%ping private` in scripts/ping.js.
const {
  ActionRowBuilder,
  ButtonBuilder,
  ButtonStyle,
...
// How long a public ping sits in its channel before it is swept. 0 keeps it
// forever. Lives only as long as the current process — see `sweep()`.
...
// Encoded into each button as `<id>:<member id>` so the handler can tell the
// newcomer from a bystander, and so the buttons keep working across a bot
// restart — a message component collector would not.
const RULES_ID = 'welcome-intro';
const PROMOTED_ID = 'welcome-member';
...
const NOT_FOR_YOU = [
  'That message is not yours — yours appeared when you joined.',
  'Dat bericht is niet van jou — die van jou verscheen toen je binnenkwam.',
].join('\n');
...
// `key` may be untracked (the #general prompt is not) — the message is deleted
// either way, and the map is only tidied when it still points at this prompt.
...
async function postPrompt(channel, member, { content, label, customId }) {
  const buttons = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`${customId}:${member.id}`)
      .setLabel(label)
...
// Step 1 — they joined. Point them at the rules.
async function greet(member) {
  ...
  const prompt = await postPrompt(channel, member, {
    content: 'welcome! Tap below before you post. · Welkom! Tik hieronder voordat je iets stuurt.',
    label: 'Read this first · Lees dit eerst',
    customId: RULES_ID,
  });
...
async function promote(member) {
  ...
  const prompt = await postPrompt(channel, member, {
    content: 'you made it — tap below. · je bent erdoor — tik hieronder.',
    label: 'Open your welcome · Open je welkom',
    customId: PROMOTED_ID,
  });

  if (prompt) sweep(`${promptKey(member)}:promoted`, prompt);
}
...
function register(client) {
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    const [name, memberId] = interaction.customId.split(':');
    if (name !== RULES_ID && name !== PROMOTED_ID) return;

    // Answer bystanders privately too, rather than leaving the click to fail.
    if (interaction.user.id !== memberId) {
      await interaction
        .reply({ content: NOT_FOR_YOU, flags: MessageFlags.Ephemeral })
        .catch(() => {});
      return;
    }

    const content = name === RULES_ID ? INTRO_TEXT : PROMOTED_TEXT;
    await interaction.reply({ content, flags: MessageFlags.Ephemeral }).catch(() => {});

    // Read and understood — take the public ping out of the channel, and drop
    // the sweep's handle on it so it does not chase a deleted message.
    if (name === RULES_ID) pendingPrompts.delete(`${interaction.guildId}:${memberId}`);
