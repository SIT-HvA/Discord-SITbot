...
const { SPACER_URL } = require('./spacer');
...
const COMPACT_TITLE_MAX = 60;
const COMPACT_LOCATION_MAX = 60;
...
// Free text from svsit.nl that lands in markdown-rendered embed text: square
// brackets could forge a clickable link and a newline would split the line.
...
function buildEventEmbed(event, { now = new Date(), description = event.description, footerNote } = {}) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Where', value: truncate(plainText(event.location || 'TBA'), FIELD_MAX), inline: true },
...
function buildEventCardEmbed(item, { now = new Date() } = {}) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Where', value: truncate(plainText(item.location || 'TBA'), FIELD_MAX), inline: true }
    )
    .setFooter({ text: footer });
...
/**
 * Builds the compact per-event embed for `/events view:compact`: a small
 * embed with the poster as a thumbnail (top right) instead of a full image,
 * so each event keeps its own category-colored border while several fit on
 * screen without a wall of full-size cards. Title (1 line) and description
 * (exactly 2 lines) are fixed-height so every card ends up the same size,
 * and the invisible spacer PNG is set as the `image` on every card: Discord
 * stretches an embed to fit its image, so this pins all compact cards to
 * the same full width alongside the fixed height.
 */
function buildEventCompactEmbed(item, { now = new Date() } = {}) {
  const category = String(item.category || 'social').toLowerCase();
  const location = truncate(plainText(item.location || 'TBA'), COMPACT_LOCATION_MAX);
  const ended = hasEnded({ date: item.date, end_date: item.dateEnd }, now) ? ' (ended)' : '';

  const embed = new EmbedBuilder()
    .setTitle(truncate(plainText(item.name), COMPACT_TITLE_MAX))
    .setURL(eventPageUrl(item.id))
    .setColor(CATEGORY_COLORS[category] ?? CATEGORY_COLORS.social)
    .setDescription(`<t:${toUnix(item.date)}:f>${ended}\n${location}`)
    .setImage(SPACER_URL);

  if (item.poster) embed.setThumbnail(item.poster);
