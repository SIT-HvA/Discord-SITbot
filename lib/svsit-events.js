...
const CARD_DESCRIPTION_MAX = 300;
const LIST_NAME_MAX = 80;
const LIST_LOCATION_MAX = 60;
const DESCRIPTION_LIMIT = 4096; // Discord embed description limit
...
/**
 * Fetches the public event list from svsit.nl for the /events overview.
 * Never throws: `{ ok: true, events }` or `{ ok: false, reason: 'unavailable' }`.
 * Note the route answers a DB failure with `[]` and status 500, so the
 * status is checked before the body is trusted.
 */
async function fetchPublicEvents({ fetch = globalThis.fetch, baseUrl = BASE_URL, timeoutMs = FETCH_TIMEOUT_MS } = {}) {
  const url = `${baseUrl.replace(/\/+$/, '')}/api/events/public`;
  ...
  try {
    ...
    const body = await response.json();
    if (!Array.isArray(body)) throw new Error('svsit.nl answered without an event list');
    // A date that does not parse would make the week filter throw later on.
    const isUsable = (item) => item && typeof item.id === 'string' && typeof item.date === 'string' && !Number.isNaN(Date.parse(item.date));
    if (!body.every(isUsable)) throw new Error('svsit.nl answered with an event missing an id or a valid date');

    return { ok: true, events: body };
  } catch (error) {
    console.error('Fetching public events from svsit.nl failed:', error);
    return { ok: false, reason: 'unavailable' };
  } finally {
    clearTimeout(timer);
  }
 ...
 * `description` replaces the event's own text (used for translations) and
 * `footerNote` is appended to the footer.
 */
function buildEventEmbed(event, { now = new Date(), description = event.description, footerNote } = {}) {
  ...
  const footer = ['svsit.nl', hasEnded(event, now) && 'This event has ended', footerNote].filter(Boolean).join('  ');
  ...
  if (description) embed.setDescription(truncate(description.trim(), DESCRIPTION_MAX));
...
/**
 * Builds the Discord embed for one item from the public event list
 * (GET /api/events/public). Same shape as `buildEventEmbed` but for the
 * list item's field names (`name`, `dateEnd`, `poster`) and without the
 * Category, Price, Signups and Tickets fields.
 */
function buildEventCardEmbed(item, { now = new Date() } = {}) {
  const category = String(item.category || 'social').toLowerCase();
  const description = item.description && item.description.trim();
  const footer = ['svsit.nl', hasEnded({ date: item.date, end_date: item.dateEnd }, now) && 'This event has ended']
    .filter(Boolean)
    .join('  ');

  const embed = new EmbedBuilder()
    .setTitle(truncate(item.name, TITLE_MAX))
    .setURL(eventPageUrl(item.id))
    .setColor(CATEGORY_COLORS[category] ?? CATEGORY_COLORS.social)
    .addFields(
      { name: 'When', value: whenField({ date: item.date, end_date: item.dateEnd }) },
      { name: 'Where', value: truncate(item.location || 'TBA', FIELD_MAX), inline: true }
    )
    .setFooter({ text: footer });

  if (description) embed.setDescription(truncate(description, CARD_DESCRIPTION_MAX));
  if (item.poster) embed.setImage(item.poster);

  return embed;
}

// Text from svsit.nl that lands inside a markdown line: square brackets would
// let it break out of, or forge, a link, and a newline would split the line.
const plainText = (text) => String(text).replace(/\[/g, '(').replace(/\]/g, ')').replace(/\s*[\r\n]+\s*/g, ' ');

/**
 * Builds one line for the compact week list: a timestamp, a link with the
 * event name and its location, plus an `(ended)` note for past events.
 */
function weekListLine(item, now) {
  const name = truncate(plainText(item.name), LIST_NAME_MAX);
  const location = truncate(plainText(item.location || 'TBA'), LIST_LOCATION_MAX);
  const ended = hasEnded({ date: item.date, end_date: item.dateEnd }, now) ? ' (ended)' : '';
  return `<t:${toUnix(item.date)}:f>  [${name}](${eventPageUrl(item.id)})  ${location}${ended}`;
}

/**
 * Builds the single compact-list embed for `/events view:compact`. Items are
 * expected to already be filtered and sorted by the caller. Lines are kept
 * in order as long as the description (including a trailing "N more" line
 * for whatever does not fit) stays under DESCRIPTION_LIMIT.
 */
function buildWeekListEmbed(items, { title, now = new Date() } = {}) {
  const lines = items.map((item) => weekListLine(item, now));

  const kept = [];
  for (const line of lines) {
    const candidate = [...kept, line];
    const rest = lines.length - candidate.length;
    const withTrailer = rest > 0 ? `${candidate.join('\n')}\nand ${rest} more on svsit.nl` : candidate.join('\n');
    if (withTrailer.length > DESCRIPTION_LIMIT) break;
    kept.push(line);
  }

  const rest = lines.length - kept.length;
  const description = rest > 0 ? `${kept.join('\n')}\nand ${rest} more on svsit.nl` : kept.join('\n');

  const embed = new EmbedBuilder().setTitle(title).setColor(CATEGORY_COLORS.social).setFooter({ text: 'svsit.nl' });
